...
class AudioPluginController(ABC):
    ...
    def setWet(self, wet: float):
        self.TrackFX.params[self.getWetIndex()] = self._param_val_calc(wet, self._wet_range[0], self._wet_range[1])

    def getWetIndex(self):
        for i, p in enumerate(self.TrackFX.params):
            if p.name == "Wet":
                return i
        
        raise ValueError("The Wet parameter's index was not found")
